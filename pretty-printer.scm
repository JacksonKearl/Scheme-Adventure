...
(define-record-type <compound-token>
    (%make-compound-token open-string close-string separator-string contents)
    compound-token?
...
(define (make-compound-token open-string close-string separator-string contents)
...
(define-generic-procedure-default-handler tokenize
  (lambda (object)
...
(define-generic-procedure-handler tokenize (match-args vector?)
  (lambda (v)
    (make-compound-token "#(" ")" "" (map tokenize (vector->list v)))))
...
(define (compound-token-width-as-column token)
  (if (= (compound-token-length token) 0)
      (compound-token-width token)
      (+ (string-length (compound-token-open-string token))
         (compute-column-width token (compound-token-contents token)))))
...
(define (compound-token-width-as-folded-column token)
  (if (<= (compound-token-length token) 1)
      ...
      (+ (string-length (compound-token-open-string token))
         ...
         space-width
         (compute-column-width token (cdr (compound-token-contents token))))))
...
(define (print-token token cursor)
  (if (or (simple-token? token)
          ...
          (and (= (compound-token-length token) 1)
               (simple-token? (car (compound-token-contents token))))
...
(define (print-open token cursor)
  (cursor-write-string (compound-token-open-string token) cursor))

(define (print-close token cursor)
  (cursor-write-string (compound-token-close-string token) cursor))

(define (print-separator token cursor)
  (cursor-write-string (compound-token-separator-string token) cursor))
...
(define (print-compound-token:as-column token cursor)
  (let* ((cursor (print-open token cursor))
    ...
    (let loop
        ((cursor (print-token (car (compound-token-contents token)) cursor))
...
(define (cursor-newline-indent target-column cursor)
  ...
  (for-each (lambda (i) i (write-char #\space (cursor-port cursor)))
            (iota target-column))
